...
void onTick(CSprite@ this)
{
	...
	if (!blob.get("BloodthirsterInfo", @bloodthirster))
	{
		return;
	}
	...
	bool grabbed=bloodthirster.state==BloodthirsterStates::grabbed;
	victim.SetVisible(grabbed);
	
	blob.Untag("attack head");
	blob.Untag("dead head");
	
	if(bloodthirster.state!=BloodthirsterStates::fatality) {
		if(bloodthirster.prevState==BloodthirsterStates::fatality) {
			/*this.ReloadSprite(MOD_PATH+"/Classes/Bloodthirster/Bloodthirster.png",this.getConsts().frameWidth,this.getConsts().frameHeight,blob.getTeamNum(),blob.getSkinNum());
			blob.Tag("reloadHead");*/
		}
		this.SetOffset(Vec2f());
	}
	if(bloodthirster.state==BloodthirsterStates::fatality) {
		if(bloodthirster.prevState!=bloodthirster.state) {
			this.SetFacingLeft(false);
			/*this.ReloadSprite(MOD_PATH+"/Classes/Bloodthirster/BloodthirsterFatality.png",this.getConsts().frameWidth,this.getConsts().frameHeight,blob.getTeamNum(),blob.getSkinNum());
			blob.Tag("reloadHead");*/
		}
	...
	}else if(bloodthirster.state==BloodthirsterStates::stun || blob.hasTag("seated")) {
	...
	else if (bloodthirster.state == BloodthirsterStates::charging)
	{
		...
		victim.SetAnimation("default");
	}else if (bloodthirster.state == BloodthirsterStates::kickAttack) {
		...
		f32 angle=	bloodthirster.attackTrueRot;
...
void onGib(CSprite@ this)
{
	...
	CParticle@ Body     = makeGibParticle("Entities/Characters/Jbloodthirster/JbloodthirsterGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 0, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Arm      = makeGibParticle("Entities/Characters/Jbloodthirster/JbloodthirsterGibs.png", pos, vel + getRandomVelocity(90, hp - 0.2 , 80), 1, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Shield   = makeGibParticle("Entities/Characters/Jbloodthirster/JbloodthirsterGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 2, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
	CParticle@ Sword    = makeGibParticle("Entities/Characters/Jbloodthirster/JbloodthirsterGibs.png", pos, vel + getRandomVelocity(90, hp + 1 , 80), 3, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);*/
}
