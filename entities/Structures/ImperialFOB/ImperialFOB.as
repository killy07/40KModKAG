...
void onInit(CBlob@ this)
{
	...
	this.set_u8("imperialfob state", FOBState::normal);
...
void onTick(CBlob@ this)
{
	...
	if (getNet().isServer())
	{
		...
		const u8 state = this.get_u8("imperialfob state");
		if (NEUTRAL_IN_WATER && isFlooded(this))
		{
			if (state != FOBState::underwater)
			{
				Capture(this, -1);
		...
		if (this.getMap().getBlobsInRadius(this.getPosition(), RAID_RADIUS, @blobsInRadius))
		{
			...
			{
				this.set_u8("imperialfob state", FOBState::raid);
				this.Tag("under raid");
			}
			...
			if (attackersCount > 0 && (friendlyCount == 0 || !not_neutral))
			{
				...
				if (!not_neutral)   // immediate capture neutral imperialfob
				{
					captureLimit = 0;
				}
				...
				this.Sync("imperialfob state", true);
	...
	if (getGameTime() % (30 * 60) == 0) {
		this.Sync("imperialfob state", true); 		// HACK: sync flooded state every minute
	}
}
...
void ReturnState(CBlob@ this)
{
	...
	u8 oldstate = this.get_u8("imperialfob state");

	u8 state = this.get_u16("tickets") > 0 ? FOBState::normal : FOBState::depleted;
	this.set_u8("imperialfob state", state);

	if (state == FOBState::normal)
		SyncFrame(this, 1);
	else
		SyncFrame(this, 3);
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	if (cmd == this.getCommandID("respawn"))
	{
		if (USE_TICKETS && getRules().isMatchRunning())
		{
			...
			if (tickets == 0)
			{
				if (this.get_u8("imperialfob state") != FOBState::underwater)
				{
					ReturnState(this);
				}
			}
	...
	{
		this.set_u8("imperialfob state", FOBState::underwater);
		this.Untag("under raid");
		this.set_s32("capture time", 0);
	}
	else if (cmd == this.getCommandID("shipment"))
	{
		...
		{
			client_AddToChat(getTranslatedString("Supplies will drop at your imperialfobs."));
		}
	}
	else
	{
...
void onChangeTeam(CBlob@ this, const int oldTeam)
{
	...
	if (this.getTeamNum() >= 0 && this.getTeamNum() < 10)
	{
		...
		if (!getRules().exists("no research"))
		{
			...
			{
				if (isFirstFOB(this, this.getTeamNum()))
				{
	...
	if (sprite !is null)
	{
		...
		else if (state == FOBState::depleted)
		{
			sprite.animation.frame = 0;
		}
		else
...
bool isFirstFOB(CBlob@ this, int team)
{
	CBlob@[] imperialfobs;
	getBlobsByName("imperialfob", @imperialfobs);
	for (uint i = 0; i < imperialfobs.length; i++)
	{
		CBlob@ imperialfob = imperialfobs[i];
		const u8 teamNumFactory = imperialfob.getTeamNum();
		if (imperialfob !is this && team == teamNumFactory)
		{
			return false;
		}
	}
