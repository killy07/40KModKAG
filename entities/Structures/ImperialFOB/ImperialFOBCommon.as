 ...
 * FOBCommon.as
 *
 * Set of common functionality for working with imperialfobs,
...
const string fob_name = "imperialfob";

const f32 BASE_RADIUS = 400.0f;
const f32 RAID_RADIUS = BASE_RADIUS * 0.25f;

const string workers_property = "imperial workers";
...
{
	if (blob.exists("normal state"))
		return (blob.get_u8("raid state") == FOBState::raid) && blob.getTeamNum() <= 10;

	return blob.hasTag("under raid");
}

shared bool isFOBDepleted(CBlob@ blob)
{
	return (blob.get_u8("depleted state") == FOBState::depleted);
}


/**
 * Update the workers of a given FOB
 */

//------------------------------------------------------//

void updateWorkers(CBlob@ imperialfob, bool raid = false)
{
	FOBWorkerSet@ workers;
	if (!imperialfob.get(workers_property, @workers) || workers is null) return;
...
/**
 * Get all applicable imperialfobs for a given blob
 *
 * Useful for getting imperialfobs to request workers from,
 * or searching for a nearby spawn.
 */
CBlob@[] getFOBsFor(CBlob@ this, f32 radius)
{
	...
	// we dont use radius here because there's likely to be less imperialfobs
	// than blobs per radius, and if there aren't then it wont't matter
	// cause it'll be some tiny number
	CBlob@[] blobs;
	if (getBlobsByName(imperialfob_name, @blobs))
	{
...
/**
 * Request a worker from a single FOB
 *
 * If you get a blob back, be sure to actually use it.
 * 		the FOB will track the caller blob so it can reclaim
 * 		on death, or with returnWorker.
 * If you get a null back, the FOB can't spare any workers.
 */
CBlob@ requestWorker(CBlob@ this, CBlob@ imperialfob)
{
	FOBWorkerSet@ workers;
	if (!imperialfob.get(workers_property, @workers) || workers is null) return null;

	return workers.getAvailableWorker(this);
}

/**
 * Request a worker from an array of imperialfobs
 *
 * Same implications
 */
CBlob@ requestWorker(CBlob@ this, CBlob@[] imperialfobs)
{
	CBlob@ b = null;
	for (uint i = 0; i < imperialfobs.length; ++i)
	{
		@b = requestWorker(this, imperialfobs[i]);
		if (b !is null)
			break;
	}
	return b;
 ...
 * False otherwise.
 */
bool returnWorker(CBlob@ this, CBlob@ imperialfob, CBlob@ worker)
{
	FOBWorkerSet@ workers;
	if (!imperialfob.get(workers_property, @workers) || workers is null) return false;
...
/**
 * Return a worker to an array of imperialfobs
 *
 * Same implications
 */
bool returnWorker(CBlob@ this, CBlob@[] imperialfobs, CBlob@ worker)
{
	for (uint i = 0; i < imperialfobs.length; ++i)
	{
		if (returnWorker(this, imperialfobs[i], worker)) return true;
	}
	return false;
}
...
shared class FOBWorker
{
	...
	/**
	 * Construct a imperialfob worker from a blob ID
	 * and an owner ID.
	 */
	FOBWorker(u16 bid, u16 oid)
	{
		Setup(bid, oid, oid);
	}

	/**
	 * Construct a imperialfob worker from an owner
	 * Automatically constructs the worker blob.
	 */
	FOBWorker(CBlob@ owner)
	{
	...
	/**
	 * Set the user of this FOB worker
	 *
	 * returns false if it's being used incorrectly - you should
	 * return workers to the imperialfob before giving them to anyone else.
	 */
	...
	/**
	 * Set the blob of this FOB worker
	 *
 ...
 * information from a imperialfob.
 *
 * You shouldn't need to modify this yourself at all,
 * just update it every now and again.
 */

shared class FOBWorkerSet
{
	FOBWorker[] workers;
	u16 ownerID;
	u32 lastRespawnTime;
	...
	FOBWorkerSet() { lastRespawnTime = 0; under_raid = false; }

	/**
	 * Construct a imperialfob worker set from an owner
	 * Automatically constructs the workers from the
	 * migrants properties vars.
	 */
	FOBWorkerSet(CBlob@ owner)
	{
		...
		for (uint i = 0 ; i < workers.length; ++i)
		{
			...
			else
			{
				...
				if (user !is null)
				{
					...
					else //they have a valid non imperialfob user
					{
						//so convert the user
...
shared void getWorkers(CBlob@ imperialfob, CBlob@[]@ workers)
{
	FOBWorkerSet@ workersSet;
	if (!imperialfob.get("imperialfob workers", @workersSet) || workersSet is null) return;

	for (uint i = 0 ; i < workersSet.workers.length; ++i)
	{
		FOBWorker@ worker = workersSet.workers[i];
		CBlob@ blob = worker.getBlob();
		workers.push_back(blob);
	}
}

shared void getFactories(CBlob@ imperialfob, CBlob@[]@ factories)
{
	FOBWorkerSet@ workersSet;
	if (!imperialfob.get("imperialfob workers", @workersSet) || workersSet is null) return;
